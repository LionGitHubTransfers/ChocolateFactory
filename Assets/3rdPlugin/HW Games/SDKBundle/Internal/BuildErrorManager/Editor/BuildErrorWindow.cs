    ...
    public class BuildErrorWindow : EditorWindow {
        ...
        internal static void LogBuildError(BuildErrorConfig.ErrorID errorID) {
            if (BuildErrorConfig.ErrorMessageDict.ContainsKey(errorID)) {
                ...
                HWLog.LogW(TAG, "Warning nonexistent ErrorID");
            }
        }
        ...
        private void OnGUI() {
            ...
            if (errorIDs.Count == 0) {
                GUILayout.Label("No HW SDKBundle Build Errors Detected");
            }
            if (errorIDs.Count > 0) {
                GUILayout.Label($"SDKBundle Build Errors ({errorIDs.Count})", EditorStyles.boldLabel);
