...
public class ObjectPool : Singleton<ObjectPool> {
    ...
    // 对象池加载(指定父物体
    public GameObject Get(object prefabName, Transform parentTrans, bool isResetPos = false) {
        return GetGO(null, prefabName, parentTrans, isResetPos);
    }

    // 对象池加载(默认父物体)
    public GameObject Get(object prefabName, bool isResetPos = false) {
        return GetGO(null, prefabName, poolTrans, isResetPos);
    }

    // 对象池加载(指定父物体)
    public GameObject Get(string path, object prefabName, Transform parentTrans, bool isResetPos = false) {
        return GetGO(path, prefabName, parentTrans, isResetPos);
    }

    // 对象池加载(默认父物体)
    public GameObject Get(string path, object prefabName, bool isResetPos = false) {
